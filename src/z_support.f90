module z_support
    ...
    subroutine read_format(USE_DIR,filename,ierr)
        ...
        if (res .eqv. .False.) then
            ...
            if(debug_z) write(*,*)'new format file: ',trim(format_file)
        endif
        ...
        open(unit=io,file=trim(filename),action='read',iostat=ierr)
            if(ierr/=0)then
                print*,'Error: failed to open format file: "'//trim(filename)//'"'
    ...
    subroutine read_eep(x)
        ...
        if (debug) print*,'reading',eepfile,x% neep,x% initial_mass,get_cols
...
!             recalibrate age from ZAMS
        ...
        deallocate(xa)
        deallocate(key_cols)
        if (debug) print*, 'Finished copying and deallocating xa'
    end subroutine
